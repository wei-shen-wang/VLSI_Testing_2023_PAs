...
public:
	...
	void tdfault_sim_a_vector(const std::string &, int &);
	void tdfault_sim_a_vector2(const std::string &, int &);
...
private:
	...
	typedef std::unique_ptr<WIRE> wptr_s;	 /* using smart pointer to hold/maintain the instances of WIRE */
	typedef std::unique_ptr<NODE> nptr_s;	 /* using smart pointer to hold/maintain the instances of NODE */
	typedef std::unique_ptr<FAULT> fptr_s; /* using smart pointer to hold/maintain the instances of FAULT */

	/* fault list */
	std::forward_list<fptr_s> flist;				/* fault list */
	std::forward_list<fptr> flist_undetect; /* undetected fault list */
	...
	std::array<std::forward_list<wptr_s>, HASHSIZE> hash_wlist; /* hashed wire list */
	std::array<std::forward_list<nptr_s>, HASHSIZE> hash_nlist; /* hashed node list */

	/* test vector  */
	int in_vector_no;									/* number of test vectors generated */
	std::vector<std::string> vectors; /* xvector set */
	...
	std::string filename;		/* current input file */
	int lineno;							/* current line number */
	std::string targv[100]; /* tokens on current command line */
	int targc;							/* number of args on current command line */
	int file_no;						/* number of current file */
	double StartTime{}, LastTime{};
	int hashcode(const std::string &);
	wptr wfind(const std::string &);
	nptr nfind(const std::string &);
	wptr getwire(const std::string &);
	nptr getnode(const std::string &);
	void newgate();
	void set_output();
	...
	std::forward_list<wptr> wlist_faulty;

	void fault_simulate_vectors(int &);
	void fault_sim_a_vector(const std::string &, int &);
	...
	public:
		...
		//  inject a fault at bit position.  Two bits (11) means this position is a fault.  When parallel fault sim, this corresponds to fault position in wire_value_f
		void inject_fault_at(int bit_position) { fault_flag |= (3 << (bit_position << 1)); }
