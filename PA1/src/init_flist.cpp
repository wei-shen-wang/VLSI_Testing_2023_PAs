...
void ATPG::generate_fault_list()
{
	...
	nptr n;
	fptr_s f;
	std::unordered_map<wptr, int> num_of_eqv_sa0, num_of_eqv_sa1;
	...
	for (auto pos : sort_wlist)
	{
		...
		f = std::move(fptr_s(new (std::nothrow) FAULT));
		if (f == nullptr)
			error("No more room!");
		f->node = n;
		...
		if (w->onode.size() > 1 || w->onode.front()->type == EQV || w->onode.front()->type == OR || w->onode.front()->type == NOR || w->onode.front()->type == XOR || w->onode.front()->type == OUTPUT)
		{
			...
			flist.push_front(std::move(f));				 // push into the fault list
		}
		else
		{
			...
			case BUF:
				for (wptr wptr_ele : w->inode.front()->iwire)
				{
					if (num_of_eqv_sa1.find(wptr_ele) == num_of_eqv_sa1.end())
						std::cerr << wptr_ele << " is not in hashmap." << std::endl;
					f->eqv_fault_num += num_of_eqv_sa1[wptr_ele];
				}
				break;
			case NAND:
	...
	for (fptr f : flist_undetect)
	{
		...
		fault_num++;
		// cout << f->fault_no << f->node->name << ":" << (f->io?"O":"I") << (f->io?9:(f->index)) << "SA" << f->fault_type << std::endl;
	}
