...
void ATPG::level_circuit()
{
	...
	bool schedule;
	int level = 0;
	std::list<nptr> event_list;					// event list linked queue
	std::forward_list<wptr> wire_stack; // stack of wires to be labeled for a given level
	...
	for (wptr wptr_ele : cktin)
	{
		...
		{
			if (std::find(event_list.begin(), event_list.end(), nptr_ele) == event_list.end()) // if this node not yet in event list
	...
	*/
	...
	{																 // go through all nodes in the event list (for this level)
		...
		if (schedule)
		{
			for (wptr wptr_ele : ncurrent->owire)
			{
				...
				{																																										 // propagate the event to fanout node
					if (std::find(event_list.begin(), event_list.end(), nptr_ele) == event_list.end()) // if not in the event list
		...
		{																																										 // if this node is not scheduled
			if (std::find(event_list.begin(), event_list.end(), ncurrent) == event_list.end()) // if not in the event list
	...
	for (wptr wptr_ele : sort_wlist)
	{
		...
		// cout << wptr_ele->name << " " << wptr_ele->level << std::endl;
	}
	// cout << "size of sort_wlist: " << sort_wlist.size() << std::endl;
} /* end of level */
		...
		{ // check every gate in the circuit
			...
			{ // check every pait of gate inputs
				for (int k = j + 1; k < n->iwire.size(); k++)
				{
					...
					{																	// if order is wrong
						std::swap(n->iwire[j], n->iwire[k]); // swap the gate inptuts
					}
				}
			}
		}
