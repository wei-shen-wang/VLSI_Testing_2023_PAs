...
int ATPG::hashcode(const std::string &name)
{
	int i = 0, j = 0;
 ...
 */
ATPG::wptr ATPG::wfind(const std::string &name)
{
	int hash_idx = hashcode(name);

	/* search the std::forward_list at hast_nlist[code],
		 using iterator to visit the elements within the std::forward_list    */
	for (const auto &pos : hash_wlist[hash_idx])
	{
		if (name == pos->name)
			return pos.get(); // "return *pos" would return a std::unique_ptr obj, that's wrong.
	}
	return (nullptr);
} /* end of wfind */
 ...
 */
ATPG::nptr ATPG::nfind(const std::string &name)
{
	int hash_idx = hashcode(name);
...
ATPG::wptr ATPG::getwire(const std::string &wirename)
{
	...
	wptr_s wtemp(new (std::nothrow) WIRE);
	if (wtemp == nullptr)
	{
		error("No more room!");
	}
	...
	hash_wlist[hash_no].push_front(std::move(wtemp)); // push into the hash table
	return hash_wlist[hash_no].front().get();
} /* end of getwire */

/* get node obj and return the node pointer */
ATPG::nptr ATPG::getnode(const std::string &nodename)
{
	...
	nptr_s ntemp(new (std::nothrow) NODE);
	if (ntemp == nullptr)
	{
		error("No more room!");
	}
	...
	hash_nlist[hash_no].push_front(std::move(ntemp)); // push into the hash table
...
void ATPG::parse_line(const std::string &line)
{
	for (auto &i : targv)
	{
		i.clear();
...
void ATPG::input(const std::string &infile)
{
	std::string line;
	filename = infile;
	std::ifstream file(filename, std::ifstream::in); // open the input vectors' file
	if (!file)
...
int ATPG::FindType(const std::string &gatetype)
{
	if (gatetype == "and")
		return (AND);
	if (gatetype == "AND")
 ...
 */
void ATPG::error(const std::string &message)
{
	fprintf(stderr, "%s around line %d in file %s\n", message.c_str(), lineno, filename.c_str());
	exit(EXIT_FAILURE);
} /* end of error */

void ATPG::timer(FILE *file, const std::string &mesg1)
{
...
void ATPG::read_vectors(const std::string &vetFile)
{
	std::string t, vec;
	size_t i;

	std::ifstream file(vetFile, std::ifstream::in); // open the input vectors' file
	if (!file)
	{ // if the ifstream obj does not exist, fail to open the file
	...
	while (!file.eof() && !file.bad())
	{
		std::getline(file, t); // get a line from the file
		if (t[0] != 'T')
		...
		else
		{
			...
			// cout << "Before erase: " << t << std::endl;
			// cout << "After erase: " << vec << std::endl;
