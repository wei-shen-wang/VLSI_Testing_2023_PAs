...
int ATPG::podem(const fptr fault, int &current_backtracks)
{
	...
	std::forward_list<wptr> decision_tree; // design_tree (a LIFO stack)
	wptr wfault;
	 ...
	 * 2. no_test
	 * 3. already std::find a test pattern AND no_of_patterns meets required total_attempt_num */
...
ATPG::wptr ATPG::test_possible(const fptr fault)
{
	...
	if (fault->node->type != OUTPUT)
	{
		...
		if (fault->node->owire.front()->value ^ U)
		{
			...
			/* std::find the next gate n to propagate, Fig 8.5*/
	...
	/* std::find a pi to achieve the objective_level on objective_wire.
...
/* std::find the eastiest propagation gate.   Fig 8.5, Fig 8.6
