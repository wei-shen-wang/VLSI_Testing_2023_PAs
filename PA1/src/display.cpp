...
void ATPG::display_undetect()
{
	int i;
	wptr w;
	std::string ufile = filename + ".uf";

	std::ofstream file(ufile, std::ifstream::out | std::ofstream::app); // open the input vectors' file
	if (!file)
	...
	for (fptr f : flist_undetect)
	{
		...
		{
			case INPUT:
				file << "primary input: " << f->node->owire.front()->name << std::endl;
				break;
			case OUTPUT:
				file << "primary output: " << f->node->iwire.front()->name << std::endl;
				break;
			default:
				file << "gate: " << f->node->name << " ;";
				if (f->io == GI)
				{
					file << "input wire name: " << f->node->iwire[f->index]->name << std::endl;
				}
				else
				{
					file << "output wire name: " << f->node->owire.front()->name << std::endl;
				}
				break;
		}
		file << "fault_type = ";
		...
		file << "fault no. = " << f->fault_no << std::endl;
		file << std::endl;
	}
	file.close();
